<template>
  ...
  <b-col  style="margin: 10px; background-color: #f8f9fa;">
            ...
            <b-row v-if="crop.plant" align-h="start" align-v="center" style="background-color: #4AAD37;padding: 10px; " class="text-white ">
               <h4 >Total Nutrients needs for {{crop.plant.name}} g/plant/day</h4>
...
console.log("ratioProduction :"+ratioProduction)
            ...
            //       = this.get_nutrientUptakeAllFases[1].expectedProductionByHa;
             ...
             for (var i = 0; i < this.plantUptakes.length; i++) {
              ...
              var days = this.plantUptakes[i].plantStage.daysOfFase;


              console.log("days :"+days)

               this.items[0].nitrogen += Number(this.plantUptakes[i].nitrogen_total)  * ratioProduction ;

               this.items[0].phosphorus += Number(this.plantUptakes[i].phosphorus_total)  * ratioProduction ;

               this.items[0].potassium += Number(this.plantUptakes[i].potassium_total)  * ratioProduction ;

               this.items[0].Magnesium += Number(this.plantUptakes[i].Magnesium_total)  * ratioProduction ;

               this.items[0].Calcium += Number(this.plantUptakes[i].Calcium_total)  * ratioProduction ;

               this.items[0].Sulfur += Number(this.plantUptakes[i].Sulfur_total)  * ratioProduction ;

               this.items[0].Sodium += Number(this.plantUptakes[i].Sodium_total)  * ratioProduction ;

               this.items[0].Boron += Number(this.plantUptakes[i].Boron_total)  * ratioProduction;

               this.items[0].zinc += Number(this.plantUptakes[i].zinc_total)  * ratioProduction ;

               this.items[0].Manganese += Number(this.plantUptakes[i].Manganese_total)  * ratioProduction;

               this.items[0].Cooper += Number(this.plantUptakes[i].Cooper_total)  * ratioProduction ;

               this.items[0].Iron += Number(this.plantUptakes[i].Iron_total)  * ratioProduction ;

               this.items[0].Aluminium += Number(this.plantUptakes[i].Aluminium_total)  * ratioProduction ;
         ...
         created(){
          console.log('Component  TotalNutrientNeeds created.');
             
            
         },
          mounted () { 

            console.log('Component  TotalNutrientNeeds mounted.');
