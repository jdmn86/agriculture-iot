...
<b-form-invalid-feedback id="input-symptoms-live-feedback">{{ validationContext.errors[0] }}</b-form-invalid-feedback>
         ...
         <ModalToConfirm  v-can="'variety-create'" v-if="plague" :name="plague.nome_comum" :isTo="'update'">
...
import ModalToConfirm from "@/components/ModalToConfirm";
  ...
  export default {
    ...
    created() {
      ...
      this.plague = Plague.query().find(this.plagueId);
      ...
      async savePlague(){      
        ...
        try {
             this.loading = true;
              ...
              const result = await Plant.api().patch('plague/'+this.plagueId,this.plague)

                // await PlantPlague.api().get('plantPlague');
                // await PlantStage.api().get('plantStage');

                this.$router.push({path: '/front/plague/'+this.plagueId});
    ...
    mounted () {
        console.log("Mounted PlagueEdit.vue");
      }
  };
  </script>
